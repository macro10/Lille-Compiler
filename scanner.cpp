 ...
 *      Author: Michael Oudshoorn/McHale Trotter
 */
...
// Processes a string. Ensures that a string begins and ends with a double quote and that the string is wholly contained on a single
// line in the source file. 2 adjacent double quote marks are interpreted as a single double quote character within the string.
...
{ 
   ...
   get_char();
   //while not at the end of the line or end of file.
   while((!eoln_flag) && (!eof_flag))
   {
      // if the next character as well as the following char are double quotes, append a double quote to the string.
      ...
      // if we find a single double quote, close the string and exit the loop.
      else if(next_char == '"')
      {  
         isClosed = true;
         break;
      }
      // if we don't find a double quote, add the next char to the string.
      else
      {
   ...
   // if the string is never closed, return an error.
   if(!isClosed)
   {
...
//Scans in a token beginning with a digit.
void scanner::scan_digit()
{

   // Initialize variables to store the numeric value and check for errors.
   int exponent = 0; 
   current_integer_value = 0;
   current_real_value = 0.0;
   string num = "";
   ...
   try
   {
      //if the string is too large to be stored as an int, return an error
      current_integer_value = stoi(num);
   }
   catch(...)
   {
   ...
   if((next_char == '.') && (following_char() != '.')) //makes sure there is no range symbol
   {
      ...
      else
      {
         while(isdigit(next_char))
         {
            //scan the part after the decimal
            num += next_char;
            digits++;
            get_char();
         }
         try
         {
            //check to see if the string is too large to store as a float
            current_real_value = stof(num);
            current_real_value = float(current_integer_value) + (current_real_value / pow(10, digits));
         }
         catch(...)
         {
            error->flag(current_line_number, current_pos_on_line, 63);
      ...
      if(next_char == 'E' || next_char == 'e')
      {
         ...
         try
         {
            //check if number is too large to store as an int
            exponent = stoi(num);
         }
         catch(...)
         {
         ...
         {
            if(sign == '+')
               current_real_value *= float(pow(10, exponent)); //formula for positive exponents
            else
               current_real_value /= float(pow(10, exponent)); //formula for negative exponents
         }
         catch(...)
         {
   ...
   else
   {
      ...
      if(next_char == 'E' || next_char == 'e')
      {
         ...
         try
         {
            //if the string is too large to be stored as an int, give an error.
            exponent = stoi(num);
         }
         catch(...)
         {
         ...
         {
            if(sign == '+')
               current_integer_value *= int(pow(10, exponent)); //formula for positive exponents
            else
               current_integer_value /= int(pow(10, exponent)); //formula for negative exponents
         }
         catch(...)
         {
            error->flag(current_line_number, current_pos_on_line, 66);
...
{
	if (recovering)
        {  //While the current sym doesn't match the parser's expected sym, and we are not at the end of the program.
           while((current_token->get_sym() != s) && (current_token->get_sym() != symbol::end_of_program))
           {
              get_token();
           }
           //Once the symbols are resynced from the recovery loop we can move to the next token and resume regular scanning/parsing.
           if(current_token->get_sym() == s)
              get_token();
        recovering = false;
        }
        //If the current symbol is the same as the parser's expected symbol.
	else if (current_token->get_sym() == s)
		get_token();
        //If the symbols do not match we throw an error and turn on the recovery flag.
	else
	{
...
//Gets the current identifier name
